...
dataGen = function(n = 200, K = 50, p = 6, p.star = 3, 
  ...
  x[x$site %in% target,2:p] = x[x$site %in% target,2:ceiling(p.star/2)] - target.delta
  x.star = x[,2:(p.star + 1)]
...
get.X.star = function(df, p, central){
  # Fits lasso using cross-validated lambda, and returns values from
  # model_params
  fit.lasso <- function(x, x.train, y.train, x.test, y.test, df.site) {
    yob_model <- cv.glmnet(x.train, y.train, alpha = 1, standardize = TRUE, nfolds = 5)
    
    X = extract.coef(yob_model)$Coefficient[3:length(extract.coef(yob_model)$Coefficient)]
    return(as.character(X))
  }
  
  # Fits SCAD using cross-validated lambda, and returns values from
  # model_params
  fit.scad <- function(x, x.train, y.train, x.test, y.test , df.site) {
    # Outcome regression
    yob_model <- cv.ncvreg(x.train, y.train, penalty="SCAD")
    
    X = names(coef(yob_model)[coef(yob_model)!=0])[c(-1,-2)]
    return(X)
  }
  
  # Fits MCP using cross-validated lambda, and returns values from
  # model_params
  fit.mcp <- function(x, x.train, y.train, x.test, y.test, df.site) {
    # Outcome regression
    yob_model <- cv.ncvreg(x.train, y.train, penalty="MCP")
    
    X = names(coef(yob_model)[coef(yob_model)!=0])[c(-1,-2)]
    return(X)
  }
  
  # Gets parameter values for each site
  param.central.site = function(r = central) {
    df.site = df[df$site==r,]
    index = sample(1:nrow(df.site), 0.7*nrow(df.site)) 
    train = df.site[index,]
    test = df.site[-index,]
    x <- model.matrix(yob ~., df[df$site==r,
    ...
    fit.l = fit.lasso(x, x.train, y.train, x.test, y.test, df.site)
    fit.s = fit.scad(x, x.train, y.train, x.test, y.test, df.site)
    fit.m = fit.mcp(x, x.train, y.train, x.test, y.test, df.site)
    
    lt = list(X_lasso = fit.l, X_scad = fit.s, X_mcp = fit.m)
    return(lt)
  }
  
  # Create matrix of parameter values for each site
  central.site.params = param.central.site()
  return(central.site.params)
}
...
tx = c(0, 10) ### treatment effect (0 is null)
...
for (i in 1:simreps){
  for(psim in p){
    ...
    cs = get.X.star(df, psim, central)
    
    test.fit.l = D.TATE(df, tar, central, cs$X_lasso, or.bias)
    sim.output[[count]] = test.fit.l
    
    test.fit.s = D.TATE(df, tar, central, cs$X_scad, or.bias)
    sim.output[[count+1]] = test.fit.s
    
    test.fit.m = D.TATE(df, tar, central, cs$X_mcp, or.bias)
    sim.output[[count+2]] = test.fit.m
    print(count)
    count = count + 3
  }
  }
...
saveRDS(sim.output, "SimOutputUnknownXStar.New.Est.RDS")
...
ate.output$n = rep(sort((rep(n, length(tx)*length(target)*3))), nrow(ate.output)/(length(n)*length(tx)*length(target)*3))
ate.output$tx = rep(sort(rep(tx, length(target)*3)), nrow(ate.output)/(length(tx)*length(target)*3))
ate.output$reg = as.factor(rep(c("lasso", "scad", "mcp"), nrow(ate.output)/3))
ate.output$setting = rep(sort(rep(seq(1:(length(p)*length(n)*length(tx)*length(target))), 3)), simreps)
...
data.plot.n = melt(ate.output, id.vars=c('setting', 'p', 'n', 'tx', 'reg'), 
...
### remove outliers
ate.output$bias.or = ifelse(ate.output$bias.or >= quantile(ate.output$bias.or, 0.75) + 1.5*IQR(ate.output$bias.or) | 
                              ate.output$bias.or <= quantile(ate.output$bias.or, 0.25) - 1.5*IQR(ate.output$bias.or), NA, ate.output$bias.or)
ate.output$bias.ipw = ifelse(ate.output$bias.ipw >= quantile(ate.output$bias.ipw, 0.75) + 1.5*IQR(ate.output$bias.ipw) |
                               ate.output$bias.ipw <= quantile(ate.output$bias.ipw, 0.25) - 1.5*IQR(ate.output$bias.ipw), NA, ate.output$bias.ipw)
ate.output$bias.dr = ifelse(ate.output$bias.dr >= quantile(ate.output$bias.dr, 0.75) + 1.5*IQR(ate.output$bias.dr) |
                              ate.output$bias.dr <= quantile(ate.output$bias.dr, 0.25) - 1.5*IQR(ate.output$bias.dr), NA, ate.output$bias.dr)
ate.output$bias.ndr = ifelse(ate.output$bias.ndr >= quantile(ate.output$bias.ndr, 0.75) + 1.5*IQR(ate.output$bias.ndr) |
                               ate.output$bias.ndr <= quantile(ate.output$bias.ndr, 0.25) - 1.5*IQR(ate.output$bias.ndr), NA, ate.output$bias.ndr)
ate.output$bias.wdr = ifelse(ate.output$bias.wdr >= quantile(ate.output$bias.wdr, 0.75) + 1.5*IQR(ate.output$bias.wdr) |
                               ate.output$bias.wdr <= quantile(ate.output$bias.wdr, 0.25) - 1.5*IQR(ate.output$bias.wdr), NA, ate.output$bias.wdr)
...
ggplot(data.plot.n) +
  geom_boxplot(aes(y=value, fill=variable, color=reg), alpha = 0.3) +
...
for(i in seq(12, 16)){
  if(count == 1){
    bias.df = aggregate(ate.output[,i] ~ p + n + tx + reg, FUN=mean, data=ate.output)
    sd.df = aggregate(ate.output[,i] ~ p + n + tx + reg, FUN=sd, data=ate.output)
    count = count + 1
  } else{
    bias.df = cbind(bias.df, aggregate(ate.output[,i] ~ p + n + tx + reg, FUN=mean, data=ate.output)[,5])
    sd.df = cbind(sd.df, aggregate(ate.output[,i] ~ p + n + tx + reg, FUN=sd, data=ate.output)[,5])
  }
}
...
write.csv(bias.df, "bias.df.reg.csv")
write.csv(sd.df, "sd.df.reg.csv")
